    ...
    for root, dirs, files in os.walk(my_face_dir):
        for file in files:
            if file.endswith("png") or file.endswith("jpg"):
                ...
                # ตรวจสอบว่ามีการตรวจจับใบหน้าในรูปภาพ
                if len(face) == 0:
...
# ตรวจสอบว่า `face` และ `ids` ไม่ว่างเปล่า
if len(face) == 0 or len(ids) == 0:
    print("Training data is empty. Please check the images and the face detection.")
else:
